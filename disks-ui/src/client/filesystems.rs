...
impl FilesystemsClient {
    ...
    pub async fn list_filesystems(&self) -> Result<Vec<FilesystemInfo>, ClientError> {
        ...
        let filesystems: Vec<FilesystemInfo> = serde_json::from_str(&json)
            .map_err(|e| ClientError::ParseError(format!("Failed to parse filesystem list: {}", e)))?;
        Ok(filesystems)
    }
    ...
    pub async fn get_supported_filesystems(&self) -> Result<Vec<String>, ClientError> {
        ...
        let types: Vec<String> = serde_json::from_str(&json)
            .map_err(|e| ClientError::ParseError(format!("Failed to parse filesystem types: {}", e)))?;
        Ok(types)
    }
    ...
    pub async fn unmount(
        ...
        let json = self.proxy.unmount(device_or_mount, force, kill_processes).await?;
        let result: UnmountResult = serde_json::from_str(&json)
            .map_err(|e| ClientError::ParseError(format!("Failed to parse unmount result: {}", e)))?;
        Ok(result)
    }
    
    /// Get processes blocking unmount
    pub async fn get_blocking_processes(&self, device_or_mount: &str) -> Result<Vec<ProcessInfo>, ClientError> {
