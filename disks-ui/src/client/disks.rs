...
impl DisksClient {
    ...
    pub async fn new() -> Result<Self, ClientError> {
        ...
        let proxy = DisksInterfaceProxy::new(&conn).await.map_err(|e| {
            ClientError::Connection(format!("Failed to create disks proxy: {}", e))
        })?;
    ...
    /// Get detailed SMART attributes
    pub async fn get_smart_attributes(&self, device: &str) -> Result<Vec<SmartAttribute>, ClientError> {
        let json = self.proxy.get_smart_attributes(device).await?;
        let attributes: Vec<SmartAttribute> = serde_json::from_str(&json)
            .map_err(|e| ClientError::ParseError(format!("Failed to parse SMART attributes: {}", e)))?;
        Ok(attributes)
    }
