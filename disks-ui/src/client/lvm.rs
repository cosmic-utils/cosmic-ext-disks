...
use storage_models::{VolumeGroupInfo, LogicalVolumeInfo, PhysicalVolumeInfo};
...
trait LvmInterface {
    ...
    #[zbus(signal)]
    async fn logical_volume_resized(&self, vg_name: &str, lv_name: &str, new_size: u64) -> zbus::Result<()>;
...
impl LvmClient {
    ...
    pub async fn new() -> Result<Self, ClientError> {
        ...
        let proxy = LvmInterfaceProxy::new(&conn).await.map_err(|e| {
            ClientError::Connection(format!("Failed to create LVM proxy: {}", e))
        })?;
    ...
    pub async fn list_volume_groups(&self) -> Result<Vec<VolumeGroupInfo>, ClientError> {
        ...
        let vgs: Vec<VolumeGroupInfo> = serde_json::from_str(&json)
            .map_err(|e| ClientError::ParseError(format!("Failed to parse volume group list: {}", e)))?;
        Ok(vgs)
    }
    
    /// List logical volumes in a volume group
    pub async fn list_logical_volumes(&self, vg_name: &str) -> Result<Vec<LogicalVolumeInfo>, ClientError> {
        let json = self.proxy.list_logical_volumes(vg_name).await?;
        let lvs: Vec<LogicalVolumeInfo> = serde_json::from_str(&json)
            .map_err(|e| ClientError::ParseError(format!("Failed to parse logical volume list: {}", e)))?;
        Ok(lvs)
    }
    ...
    pub async fn list_physical_volumes(&self) -> Result<Vec<PhysicalVolumeInfo>, ClientError> {
        ...
        let pvs: Vec<PhysicalVolumeInfo> = serde_json::from_str(&json)
            .map_err(|e| ClientError::ParseError(format!("Failed to parse physical volume list: {}", e)))?;
        Ok(pvs)
    }
    
    /// Create a new volume group from devices
    pub async fn create_volume_group(&self, name: &str, devices: &[String]) -> Result<(), ClientError> {
    ...
    pub async fn create_logical_volume(
        ...
        Ok(self.proxy.create_logical_volume(vg_name, lv_name, size).await?)
    }
    ...
    pub async fn resize_logical_volume(
        ...
        Ok(self.proxy.resize_logical_volume(vg_name, lv_name, new_size).await?)
    }
    ...
    /// Delete a logical volume
    pub async fn delete_logical_volume(&self, vg_name: &str, lv_name: &str) -> Result<(), ClientError> {
        Ok(self.proxy.delete_logical_volume(vg_name, lv_name).await?)
    }
    
    /// Remove a physical volume from a volume group
    pub async fn remove_physical_volume(&self, vg_name: &str, device: &str) -> Result<(), ClientError> {
