...
## Context
The Disk menu in the UI is already wired to `Message` variants, but several actions currently show “not implemented yet” dialogs in `disks-ui/src/app.rs`. Additionally, image-related actions currently live under the Image menu even when they conceptually operate on the selected drive.
...
- Update `disks-ui/src/views/menu.rs`:
...
- Update `Message` handling in `disks-ui/src/app.rs`:
  - Remove or deprecate `DriveSettings` flow.
  - Replace “not implemented yet” dialogs for Disk actions with real async tasks.

### 2) DBus backend surface in `disks-dbus`
Implement missing drive operations in `disks-dbus/src/disks/drive.rs` (or a dedicated module) using UDisks2:
...
- The `udisks2` crate may not expose all ATA calls; use a raw `zbus::Proxy` call when needed (pattern already used in `disks-dbus/src/disks/ops.rs` for better error messages).
...
### 3) UI flows
- Add dialogs/pages in `disks-ui/src/views/dialogs.rs` or new view modules:
