# GAP-001.b: Complete disks-dbus Restructuring - Flat Domain-Based Architecture
...
Currently, `disks-dbus` has **multiple inconsistent architectural patterns** scattered across the codebase:

### Current State (Inconsistent)

1. **operations/** (created in GAP-001) - ✅ Clean flat functions
...
2. **disks/drive/** - ⚠️ OOP methods on DriveModel (6 files, 1,453 lines)
...
3. **disks/volume_model/** - ⚠️ OOP methods on VolumeModel (7 files, 994 lines)
...
4. **disks/** - ⚠️ Mixed patterns (standalone modules)
...
**Complete restructuring** of disks-dbus into consistent domain-based flat function modules. Remove ALL duplicates by merging and keeping the best implementations.

### New Architecture (Consistent & Complete)

```
disks-dbus/src/
├── lib.rs                    (exports everything at top level)
│
...
│   ├── mod.rs
│   ├── discovery.rs          (get_drives, get_disks, enumerate)
│   ├── format.rs             (format_disk, create_partition_table)
│   ├── power.rs              (eject, power_off, standby, wakeup)
│   ├── model.rs              (DriveModel struct - data only)
│   └── volume_tree.rs        (internal: volume hierarchy building)
│
├── partition/                🆕 Partition operations
│   ├── mod.rs
│   ├── create.rs             (create_partition)
│   ├── delete.rs             (delete_partition)
│   ├── edit.rs               (set_type, set_name, set_flags, edit_all)
│   ├── resize.rs             (resize_partition)
│   └── info.rs               (partition_info, flags helpers)
│
├── filesystem/               🆕 Filesystem operations
│   ├── mod.rs
│   ├── format.rs             (format_filesystem)
│   ├── mount.rs              (mount, unmount)
│   ├── check.rs              (check_filesystem, repair_filesystem)
│   ├── label.rs              (get_label, set_label)
│   ├── ownership.rs          (take_ownership)
│   └── config.rs             (mount options management)
│
├── encryption/               🆕 LUKS/encryption operations
│   ├── mod.rs
│   ├── format.rs             (format_luks)
│   ├── unlock.rs             (unlock_luks)
│   ├── lock.rs               (lock_luks)
│   ├── passphrase.rs         (change_passphrase)
│   ├── list.rs               (list_luks_devices)
│   └── config.rs             (encryption options - crypttab management)
│
├── image/                    🆕 Image/backup operations
│   ├── mod.rs
│   ├── loop.rs               (loop_setup, loop_delete)
│   ├── mount.rs              (mount_filesystem for images)
│   └── backup.rs             (open_for_backup, open_for_restore)
│
├── smart/                    🆕 SMART monitoring
│   ├── mod.rs
│   ├── info.rs               (get_smart_info)
│   ├── test.rs               (start_selftest, abort_selftest)
│   └── types.rs              (SmartInfo, SmartSelfTestKind definitions)
│
├── lvm/                      🆕 LVM operations
│   ├── mod.rs
│   ├── list.rs               (list_lvs_for_pv)
│   └── (future: create_lv, delete_lv, etc.)
│
├── btrfs/                    🆕 BTRFS-specific operations
│   ├── mod.rs
│   └── types.rs              (BtrfsFilesystem, BtrfsSubvolume)
│
├── gpt/                      🆕 GPT utilities
│   ├── mod.rs
│   └── probe.rs              (probe_gpt_usable_range_bytes, fallback)
│
├── manager/                  🆕 Device event monitoring
│   ├── mod.rs
│   ├── events.rs             (DeviceEvent, DeviceEventStream)
│   └── disk_manager.rs       (DiskManager)
│
├── volume/                   🆕 Volume hierarchy
│   ├── mod.rs
│   ├── node.rs               (VolumeNode type)
│   ├── index.rs              (BlockIndex)
│   └── model.rs              (VolumeModel struct - data only)
│
└── util/                     🆕 Shared utilities
    ├── mod.rs
    ├── bytestring.rs         (D-Bus bytestring helpers)
    ├── path.rs               (path conversion helpers)
    └── process.rs            (find_processes_using_mount, kill_processes)
```
...
```bash
mkdir -p disks-dbus/src/{disk,partition,filesystem,encryption,image,smart,lvm,btrfs,gpt,manager,volume,util}
```

2. Create initial `mod.rs` files for each domain

3. Move type definitions:
   - `disks/smart.rs` types → `smart/types.rs`
   - `disks/btrfs_native.rs` types → `btrfs/types.rs`
   - `disks/volume.rs` types → `volume/node.rs` + `volume/index.rs`
   - Create `types.rs` at root for shared types

4. Move model structs (data-only):
   - `disks/drive/model.rs` → `disk/model.rs` (strip all methods)
   - `disks/volume_model/mod.rs` VolumeModel → `volume/model.rs` (strip all methods)
...
| edit_partition (all-in-one) | ❌ | volume_model/partition.rs | **Keep** - useful convenience wrapper |
...
| repair_filesystem | ❌ | volume_model/filesystem.rs | Keep volume_model |
| set_label | operations/filesystems.rs | volume_model/filesystem.rs | Keep operations |
| get_label | operations/filesystems.rs | ❌ | Keep operations |
| take_ownership | operations/filesystems.rs | volume_model/filesystem.rs | Keep operations |
| get_mount_point | operations/filesystems.rs | ❌ | Keep operations |
...
| Operation | Source 1 (operations) | Source 2 (volume_model) | Decision |
|-----------|----------------------|------------------------|----------|
| format_luks | operations/luks.rs | ❌ | Keep operations |
...
| list_luks_devices | operations/luks.rs | ❌ | Keep operations |
| get_encryption_options | ❌ | volume_model/encryption.rs | Keep volume_model |
| set_encryption_options | ❌ | volume_model/encryption.rs | Keep volume_model |
...
1. **storage-service:** Should require minimal changes (already uses operations module)
   - Update imports: `use disks_dbus::operations::*` → `use disks_dbus::*`
   - Already using flat function API

2. **disks-ui:** May need updates if it uses DriveModel/VolumeModel methods
   - Search and replace method calls with function calls
   - `drive.eject()` → `disk::eject_drive(drive.path)`
   - `volume.mount()` → `filesystem::mount_filesystem(&volume.device_path)`
...
1. **Remove Backend Trait Abstraction** (2-3 hours)
   ...
   **Files to delete entirely:**
   ```bash
   rm disks-dbus/src/disks/ops.rs        # Contains DiskBackend trait, RealDiskBackend, FakeBackend, + 300 lines of tests
   ```
...
```bash
rm -rf disks-dbus/src/operations/     # Merged into domain modules
rm -rf disks-dbus/src/disks/drive/    # Merged into disk/
rm -rf disks-dbus/src/disks/volume_model/  # Merged into volume/, partition/, filesystem/, encryption/
rm -rf disks-dbus/src/disks/          # Everything moved out
```

3. **Delete old files:**
```bash
rm disks-dbus/src/disks/image.rs      # → image/
rm disks-dbus/src/disks/lvm.rs        # → lvm/
rm disks-dbus/src/disks/gpt.rs        # → gpt/
rm disks-dbus/src/disks/smart.rs      # → smart/types.rs
rm disks-dbus/src/disks/process_finder.rs  # → util/process.rs
rm disks-dbus/src/disks/volume.rs     # → volume/node.rs
rm disks-dbus/src/disks/mod.rs        # No longer needed
...
6. **Documentation:**
   - Update disks-dbus README with new structure
...
|-----------|------------|---------------|--------|------------|
| **Partitions** | | | | |
| create_table | ✅ partitions.rs | | ✅ format_disk | **Keep operations** - direct |
| create | ✅ partitions.rs | | ✅ actions.rs | **Keep operations** |
| delete | ✅ partitions.rs | ✅ partition.rs | | **Merge** - add unmount logic |
| resize | ✅ partitions.rs | ✅ partition.rs | | **Keep operations** |
| set_type | ✅ partitions.rs | ✅ partition.rs | | **Keep operations** |
| set_name | ✅ partitions.rs | ✅ partition.rs | | **Keep operations** |
| set_flags | ✅ partitions.rs | ✅ partition.rs | | **Keep operations** |
| edit (all-in-one) | | ✅ partition.rs | | **Keep volume_model** |
| **Filesystems** | | | | |
| format | ✅ filesystems.rs | ✅ filesystem.rs | | **Keep operations** |
| mount | ✅ filesystems.rs | ✅ mount.rs | | **Keep operations** |
| unmount | ✅ filesystems.rs | ✅ mount.rs | | **Keep operations** |
| check | ✅ filesystems.rs | ✅ filesystem.rs | | **Keep operations** |
| repair | | ✅ filesystem.rs | | **Keep volume_model** (wrapper) |
| get_label | ✅ filesystems.rs | | | **Keep operations** |
| set_label | ✅ filesystems.rs | ✅ filesystem.rs | | **Keep operations** |
| take_ownership | ✅ filesystems.rs | ✅ filesystem.rs | | **Keep operations** |
| get_mount_point | ✅ filesystems.rs | | | **Keep operations** |
| mount_options | | ✅ mount.rs + config.rs | | **Keep volume_model** |
| **Encryption** | | | | |
| format_luks | ✅ luks.rs | | | **Keep operations** |
| unlock | ✅ luks.rs | ✅ encryption.rs | | **Keep operations** |
| lock | ✅ luks.rs | ✅ encryption.rs | | **Keep operations** |
| change_passphrase | ✅ luks.rs | ✅ encryption.rs | | **Keep operations** |
| list_devices | ✅ luks.rs | | | **Keep operations** |
| encryption_options | | ✅ encryption.rs | | **Keep volume_model** |
| **Image/Backup** | | | | |
| open_for_backup | | ✅ backup.rs | ✅ actions.rs | **Merge** - both needed |
| open_for_restore | | ✅ backup.rs | ✅ actions.rs | **Merge** - both needed |
...
### Risk 1: Breaking disks-ui during transition
...
1. ✅ All operations moved to domain modules (disk/, partition/, filesystem/, encryption/, image/, smart/, lvm/, etc.)
2. ✅ All duplicates resolved - single implementation per operation
3. ✅ DriveModel and VolumeModel are data-only (no methods except constructors)
4. ✅ `/operations/` directory deleted (merged into domain modules)
5. ✅ `/disks/drive/` directory deleted (merged into disk/)
6. ✅ `/disks/volume_model/` directory deleted (merged into domain modules)
7. ✅ All old standalone modules moved (image, lvm, gpt, smart, etc.)
8. ✅ All operations exported at top level in lib.rs
9. ✅ storage-service builds and runs (minimal/no changes expected)
10. ✅ disks-ui builds and runs (method calls converted to functions)
11. ✅ Full workspace compiles: `cargo build --workspace`
12. ✅ All tests pass: `cargo test --workspace`
13. ✅ Smoke tests pass (manual testing of key operations)
14. ✅ Documentation updated (README, migration guide)
