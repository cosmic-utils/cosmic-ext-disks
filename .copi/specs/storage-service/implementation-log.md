# Implementation Log: Phase 3A - Refactor storage-dbus
...
3. **storage-service/src/conversions.rs** (TEMPORARY FIX)
   ...
   - **Added deprecation note** - This file will be removed in Task 6 when storage-dbus returns storage-models types directly

### Architecture Decisions

1. **Single Source of Truth**: storage-models types are THE domain models, not transport types
   - storage-dbus will return these types directly (Task 6)
   - storage-service serializes/deserializes them (no conversion)
   - storage-ui uses them, optionally wrapping for UI state
...
- ⚠️  13 warnings in storage-ui (existing, unrelated to changes)
- ⚠️  2 warnings in storage-dbus (existing, unrelated to changes)
...
- **Task 12**: Update storage-ui imports (apply {Type}Model naming)
...
- ByteRange and Usage moved from storage-dbus (will update storage-dbus to import from storage-models in Task 6)
...
### Objective
Update storage-dbus to use storage-models types as its public API, eliminating the need for conversions in storage-service.

### Files Modified

#### Modified Files
1. **storage-dbus/Cargo.toml**
   - Added `storage-models = { path = "../storage-models" }` dependency
   
2. **storage-dbus/src/disks/drive/model.rs**
...
3. **storage-dbus/src/disks/drive/discovery.rs**
...
4. **storage-dbus/src/disks/gpt.rs**
...
**Conversion Strategy:**
- Kept DriveModel internal (contains zbus::Connection, needs to stay in storage-dbus)
...
**Type Consolidation:**
- ByteRange: Was duplicated in storage-dbus/src/disks/gpt.rs and storage-models
  - Now single definition in storage-models with helper methods
- Usage: Was in storage-dbus/src/usage.rs, now re-exported from storage-models
...
### Build Status
- ✅ `cargo check -p cosmic-ext-storage-dbus` - SUCCESS
- ✅ `cargo check --workspace` - SUCCESS
- ⚠️  3 warnings in storage-dbus (existing, unrelated GetDefault/stime/rtime dead code)
- ⚠️  13 warnings in storage-ui (existing, needs update in Task 12)

### Next Steps
- **Task 7**: Refactor VolumeNode to return storage-models::VolumeInfo
- **Task 12**: Update storage-ui to use DriveModel::get_disks() → storage_models::DiskInfo
- **Phase 3B**: storage-service can now remove conversions.rs entirely (DriveModel → DiskInfo handled in storage-dbus)

### Blockers
None

### Notes
- DriveModel will remain internal to storage-dbus (needed for connection management)
- storage-service conversions.rs can be deleted once storage-ui is updated
...
1. **storage-dbus/src/disks/mod.rs**
   - Temporarily re-exported DriveModel, VolumeModel, VolumeNode, VolumeKind, VolumeType for storage-ui compatibility
   - Re-exported find_processes_using_mount and kill_processes

2. **storage-dbus/src/disks/process_finder.rs**
...
3. **storage-dbus/src/disks/drive/actions.rs**
   - Fixed import: `use crate::disks::volume::VolumeNode;`

4. **storage-dbus/src/disks/drive/model.rs**
   - Fixed import: `use crate::disks::{VolumeModel, volume::VolumeNode};`

5. **storage-dbus/src/disks/drive/volume_tree.rs**
   - Fixed import: `use crate::disks::{BlockIndex, volume::{VolumeKind, VolumeNode}};`

6. **storage-dbus/src/lib.rs**
   - Re-exported `storage_models::ProcessInfo` for storage-ui

### Architecture Status

**Current State (Transitional):**
- storage-dbus public API includes both old (DriveModel, VolumeNode, etc.) and new (storage_models) types
- Internal types use full module paths
- ProcessInfo/KillResult consolidated to storage-models

**Legacy Code Exposed (Temporary):**
- DriveModel - storage-ui uses get_drives() extensively
- VolumeModel, VolumeNode - storage-ui uses for UI tree
- VolumeKind, VolumeType - storage-ui enums
- find_processes_using_mount, kill_processes - storage-ui mount operations

**Next Phase (Task 12):**
- Update storage-ui to use storage_models::DiskInfo instead of DriveModel
- Update storage-ui to use storage_models::VolumeInfo instead of VolumeNode
- Remove DriveModel, VolumeNode, VolumeModel from public API
- Keep only storage-models types public

### Build Status
- ✅ `cargo check --workspace` - SUCCESS
- ⚠️  13 warnings in storage-ui (existing, unrelated)
- ⚠️  3 warnings in storage-service (unused conversions now)

### Blockers
None - storage-ui needs refactoring but compiles with transitional API
...
1. **storage-dbus/src/disks/volume.rs**
...
2. **storage-dbus/src/disks/volume_model/mod.rs**
...
3. **storage-dbus/src/disks/drive/model.rs**
...
- **Task 12**: Update storage-ui to use new API methods
...
**Files Modified:**
1. **storage-dbus/src/disks/lvm.rs**
...
- No duplicated LUKS types in storage-dbus
...
**Phase 3A Goal:** Make storage-models the single source of truth by having storage-dbus return these types directly.
...
- Moved ByteRange from storage-dbus/gpt.rs to storage-models (eliminated duplicate)
- Moved Usage from storage-dbus/usage.rs to storage-models
- Moved ProcessInfo, KillResult from storage-dbus/process_finder.rs to storage-models
...
UDisks2 (raw D-Bus API)
    ↓
storage-dbus internal types (DriveModel, VolumeNode, VolumeModel)
    ├─ Contains zbus::Connection (operational handles)
    └─ impl From conversions
    ↓
storage-dbus public API (BOTH available):
...
- Modified: 10 storage-dbus files for conversions and type consolidation
- Tests: All passing (2 LVM tests verified)

### Build Status
```
✅ cargo check --workspace - 0.57s
✅ cargo test -p cosmic-ext-storage-dbus --lib lvm - 2/2 passed
...
**Rationale:** Tasks 12-15 attempt to transition storage-ui to storage-models types before storage-service exists. This creates an awkward hybrid architecture:
- UI would use storage-models for display
- But still need storage-dbus DriveModel for operations (needs zbus::Connection)
- Defeats the clean separation goal

**Recommended Path Forward:**
1. ✅ Phase 3A Tasks 1-11: COMPLETE - storage-dbus returns storage-models types
2. **Next: Phase 3B** - Implement storage-service D-Bus interface
   - Service wraps storage-dbus internally
   - Exposes  operations via D-Bus methods
   - Returns storage-models types via D-Bus
3. **Then: Update storage-ui** (combines old Tasks 12-15)
   - Replace storage-dbus dependency with storage-service client
...
- Wrap storage-dbus operations
- Expose storage-models over D-Bus
- ~8-10 weeks (57 tasks per original spec)

**Option B:** Force Tasks 12-15 now (transitional hybrid)
- Update storage-ui to prefer storage-models for display
- Keep storage-dbus DriveModel for operations
...
**Objective:** Add storage-dbus dependency and create DisksHandler structure
...
**Dependencies:**
- ✅ storage-dbus already in Cargo.toml
...
**Implementation Details:**
- Calls `DriveModel::get_disks()` from storage-dbus (Phase 3A API)
...
- No double conversion - storage-dbus returns storage-models types directly
