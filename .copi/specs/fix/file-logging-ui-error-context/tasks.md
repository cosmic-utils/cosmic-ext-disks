...
## Task 1: Add file-based tracing to disks-ui
- Scope: persistent log file output (daily rolling; keep 7 days) + env-configurable filters.
- Likely files:
  - `disks-ui/src/main.rs`
  - new: `disks-ui/src/logging.rs` (or similar)
...
- Likely files:
  - `disks-ui/src/ui/volumes/update/{partition,create,filesystem,encryption,mount_options}.rs`
  - `disks-ui/src/ui/app/update/{drive,image,image/dialogs}.rs`
  - new small helper module (e.g. `disks-ui/src/ui/error.rs` or `disks-ui/src/utils/report.rs`)
- Steps:
...
## Task 3: Preserve UDisks2 method error messages in disks-dbus operations
- Scope: prevent “The operation failed” from being the only detail.
- Likely files:
  - `disks-dbus/src/disks/ops.rs`
  - potentially `disks-dbus/src/disks/{volume,volume_model,manager}.rs` (call sites)
- Steps:
...
- Test plan:
  ...
  - Run existing unit tests (`disks-dbus` has tests in `ops.rs`) and add minimal new tests around error formatting if feasible (mock backend returning a method-error-shaped error).
- Done when:
...
- Likely files:
  - `README.md` (top-level or `disks-ui/README.md`)
- Steps:
