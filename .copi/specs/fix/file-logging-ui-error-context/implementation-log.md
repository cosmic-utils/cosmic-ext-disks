...
### Summary
- Added daily-rotating file logging for `storage-ui` with 7-day best-effort retention cleanup.
- Ensured every surfaced UI error dialog created from a backend `Err(e)` also logs via `log_error_and_show_dialog`.
- Preserved UDisks2 `MethodError` name/message for key operations by switching to raw `zbus::Proxy` calls in `storage-dbus`.
- Documented log location and environment overrides.

### Notable changes
- `storage-ui/src/logging.rs`: stdout + daily file logs; `RUST_LOG` support; `COSMIC_EXT_DISKS_LOG_DIR`/`COSMIC_EXT_DISKS_LOG_FILE` overrides; retention cleanup.
- `storage-ui/src/ui/error.rs`: `log_error_and_show_dialog` helper (only allowed wrapper around logging).
- UI migrations:
  - `storage-ui/src/ui/volumes/update/{mount_options,create,filesystem,encryption,partition}.rs`
  - `storage-ui/src/ui/app/update/drive.rs`
- `storage-dbus/src/disks/ops.rs`: raw zbus calls + consistent formatting for `MethodError` details for:
