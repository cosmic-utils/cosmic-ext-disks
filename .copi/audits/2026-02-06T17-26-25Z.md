# Code Quality Audit — disks-ui (cosmic-ext-disks)
...
**Scope:** Code quality audit focused on `disks-ui/` module (UI crate only)  
**Lines of Code:** ~8,742 lines of Rust in disks-ui/src
...
### Entrypoints
- [disks-ui/src/main.rs](../disks-ui/src/main.rs) — application bootstrap
- [disks-ui/src/app.rs](../disks-ui/src/app.rs) — public API re-exports

### Core State
- [disks-ui/src/ui/app/state.rs](../disks-ui/src/ui/app/state.rs) — `AppModel` (COSMIC application state)
- [disks-ui/src/ui/volumes/state.rs](../disks-ui/src/ui/volumes/state.rs) — `VolumesControl` and `Segment` types
- [disks-ui/src/ui/dialogs/state.rs](../disks-ui/src/ui/dialogs/state.rs) — 17 dialog state structs
- [disks-ui/src/ui/sidebar/state.rs](../disks-ui/src/ui/sidebar/state.rs) — sidebar tree state

### Update Logic (Message Handlers)
- [disks-ui/src/ui/app/update/mod.rs](../disks-ui/src/ui/app/update/mod.rs) (306 lines) — top-level message dispatcher
- [disks-ui/src/ui/volumes/update/*.rs](../disks-ui/src/ui/volumes/update/) — 7 modules handling volume operations
  - partition.rs (353 lines), encryption.rs (498 lines), mount_options.rs (387 lines)

### Views
- [disks-ui/src/ui/app/view.rs](../disks-ui/src/ui/app/view.rs) (809 lines) — main view builder, dialog dispatcher
- [disks-ui/src/ui/volumes/view.rs](../disks-ui/src/ui/volumes/view.rs) (349 lines) — segment visualization and volume rows
- [disks-ui/src/ui/sidebar/view.rs](../disks-ui/src/ui/sidebar/view.rs) (465 lines) — sidebar tree rendering
- [disks-ui/src/ui/dialogs/view/*.rs](../disks-ui/src/ui/dialogs/view/) — dialog UI builders

### Utilities
- [disks-ui/src/utils/segments.rs](../disks-ui/src/utils/segments.rs) (453 lines) — disk segmentation logic
- [disks-ui/src/ui/volumes/helpers.rs](../disks-ui/src/ui/volumes/helpers.rs) — volume tree traversal helpers
- [disks-ui/src/utils/ui.rs](../disks-ui/src/utils/ui.rs) (210 lines) — UI helper widgets
...
**Evidence:**
- 50+ instances found via `grep '\.clone\(\)'` across disks-ui/src
- Particularly prevalent in:
  - [disks-ui/src/ui/app/update/drive.rs](../disks-ui/src/ui/app/update/drive.rs) L29-32, L46, L73-74, L109-110, etc.
  - [disks-ui/src/ui/app/update/smart.rs](../disks-ui/src/ui/app/update/smart.rs) L9, L33, L35, L37, L47, etc.
  - [disks-ui/src/ui/sidebar/view.rs](../disks-ui/src/ui/sidebar/view.rs) L178, L189, L220, L231, L266, etc.
  - [disks-ui/src/ui/volumes/update/mount.rs](../disks-ui/src/ui/volumes/update/mount.rs) L11, L41
  - [disks-ui/src/ui/app/update/nav.rs](../disks-ui/src/ui/app/update/nav.rs) L15, L45, L63, L78, L83-84, L89, L95-96, etc.

**Examples:**

```rust
// disks-ui/src/ui/app/update/drive.rs L29-32
...
```rust
// disks-ui/src/ui/volumes/update/mount.rs L11
...
[disks-ui/src/ui/volumes/update/mount.rs](../disks-ui/src/ui/volumes/update/mount.rs) L18-30:
```rust
|result| match result {
...
vs. [disks-ui/src/ui/volumes/update/partition.rs](../disks-ui/src/ui/volumes/update/partition.rs) L70-80:
```rust
|result| match result {
    Ok(drives) => Message::UpdateNav(drives, None).into(),
    Err(e) => {
...
vs. [disks-ui/src/ui/volumes/update/partition.rs](../disks-ui/src/ui/volumes/update/partition.rs) L300-305:
```rust
|result| match result {
    Ok(drives) => Message::UpdateNav(drives, None).into(),
    Err(e) => {
...
**Additional Evidence:**
- [disks-ui/src/ui/app/update/mod.rs](../disks-ui/src/ui/app/update/mod.rs) L56: `tracing::warn!("received volumes message with no active VolumesControl");` — warning but no user feedback
- [disks-ui/src/ui/app/update/mod.rs](../disks-ui/src/ui/app/update/mod.rs) L67-70: early return with `Task::none()` on error — silent failure
- [disks-ui/src/ui/app/view.rs](../disks-ui/src/ui/app/view.rs) L55: `match open::that_detached(&url) { Ok(()) => {} Err(err) => { tracing::warn!(...) } }` — log but no dialog
...
**Evidence:**
```rust
// disks-ui/src/ui/volumes/state.rs L13
...
```rust
// disks-ui/src/ui/volumes/state.rs L18
#[derive(Clone, Debug)]
#[allow(dead_code)]
pub struct Segment { /* ... */ }
```
...
```rust
// disks-ui/src/ui/volumes/state.rs L82
#[allow(dead_code)]
...
```rust
// disks-ui/src/ui/app/message.rs L14
...
`OpenPath` marked temporarily unused but is actually used in [disks-ui/src/ui/app/update/mod.rs](../disks-ui/src/ui/app/update/mod.rs) L37 and [disks-ui/src/ui/app/view.rs](../disks-ui/src/ui/app/view.rs) L364.

```rust
// disks-ui/src/ui/dialogs/state.rs L4
#[allow(dead_code)]
pub enum ShowDialog { /* ... */ }
```
...
```rust
// disks-ui/src/ui/volumes/view.rs L224
#[allow(dead_code)]
...
```rust
// disks-ui/src/ui/dialogs/message.rs L81
#[allow(dead_code)]
Continue,
```
...
**Acceptance Criteria:**
- [ ] Zero `#[allow(dead_code)]` attributes in disks-ui
...
[disks-ui/src/ui/dialogs/state.rs](../disks-ui/src/ui/dialogs/state.rs) defines 17 separate dialog structs:
...
[disks-ui/src/ui/app/view.rs](../disks-ui/src/ui/app/view.rs) L29-119 contains 17 match arms that just delegate to dialog view functions:
...
**Evidence:**
[disks-ui/src/ui/app/view.rs](../disks-ui/src/ui/app/view.rs) contains:
...
[disks-ui/src/ui/volumes/update/mount.rs](../disks-ui/src/ui/volumes/update/mount.rs) contains 4 nearly identical functions:
...
[disks-ui/src/ui/volumes/view.rs](../disks-ui/src/ui/volumes/view.rs) L68-170:
```rust
...
[disks-ui/src/ui/app/update/mod.rs](../disks-ui/src/ui/app/update/mod.rs) L132-180: Complex nested volume selection logic with 6 levels of nesting.
...
**Single Stale TODO:**
[disks-ui/src/ui/app/update/mod.rs](../disks-ui/src/ui/app/update/mod.rs) L75:
```rust
...
**Unneeded Comment:**
[disks-ui/src/ui/app/update/image/ops.rs](../disks-ui/src/ui/app/update/image/ops.rs) L77:
```rust
...
**Unhelpful Comment:**
[disks-ui/src/ui/app/view.rs](../disks-ui/src/ui/app/view.rs) L135:
```rust
...
**Validation TODO:**
[disks-ui/src/utils/ui.rs](../disks-ui/src/utils/ui.rs) L30:
```rust
...
[disks-ui/src/ui/volumes/state.rs](../disks-ui/src/ui/volumes/state.rs) L195-211:
```rust
...
[disks-ui/src/ui/volumes/view.rs](../disks-ui/src/ui/volumes/view.rs) L74-80:
```rust
...
[disks-ui/src/ui/volumes/disk_header.rs](../disks-ui/src/ui/volumes/disk_header.rs) L21-31:
```rust
...
1. **Remove unused `tooltip_icon_button` function** ([disks-ui/src/ui/volumes/view.rs](../disks-ui/src/ui/volumes/view.rs) L224-241) — 18 lines saved
...
1. **Is `DeviceManager.apply_change()` still a planned feature?** ([disks-ui/src/ui/app/update/mod.rs](../disks-ui/src/ui/app/update/mod.rs) L75) — no evidence in codebase
...
- [disks-ui/src/ui/app/state.rs](../disks-ui/src/ui/app/state.rs)
- [disks-ui/src/ui/app/update/mod.rs](../disks-ui/src/ui/app/update/mod.rs)
- [disks-ui/src/ui/app/update/drive.rs](../disks-ui/src/ui/app/update/drive.rs)
- [disks-ui/src/ui/app/update/nav.rs](../disks-ui/src/ui/app/update/nav.rs)
- [disks-ui/src/ui/app/update/smart.rs](../disks-ui/src/ui/app/update/smart.rs)
- [disks-ui/src/ui/volumes/state.rs](../disks-ui/src/ui/volumes/state.rs)
- [disks-ui/src/ui/volumes/update/mount.rs](../disks-ui/src/ui/volumes/update/mount.rs)
- [disks-ui/src/ui/volumes/update/partition.rs](../disks-ui/src/ui/volumes/update/partition.rs)
- [disks-ui/src/ui/dialogs/state.rs](../disks-ui/src/ui/dialogs/state.rs)
- [disks-ui/src/ui/sidebar/state.rs](../disks-ui/src/ui/sidebar/state.rs)

View and rendering:
- [disks-ui/src/ui/app/view.rs](../disks-ui/src/ui/app/view.rs)
- [disks-ui/src/ui/volumes/view.rs](../disks-ui/src/ui/volumes/view.rs)
- [disks-ui/src/ui/volumes/disk_header.rs](../disks-ui/src/ui/volumes/disk_header.rs)
- [disks-ui/src/ui/sidebar/view.rs](../disks-ui/src/ui/sidebar/view.rs)
- [disks-ui/src/ui/dialogs/view/partition.rs](../disks-ui/src/ui/dialogs/view/partition.rs)
- [disks-ui/src/ui/dialogs/view/common.rs](../disks-ui/src/ui/dialogs/view/common.rs)

Utilities:
- [disks-ui/src/utils/segments.rs](../disks-ui/src/utils/segments.rs)
- [disks-ui/src/ui/volumes/helpers.rs](../disks-ui/src/ui/volumes/helpers.rs)
- [disks-ui/src/utils/ui.rs](../disks-ui/src/utils/ui.rs)
