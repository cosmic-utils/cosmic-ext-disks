...
1. **CRITICAL: Architectural Layer Violation** — `storage-service` directly imports and calls `storage-dbus` internal types (30+ callsites), violating the intended architecture where `storage-service` should only depend on `storage-models`
...
5. **HIGH: Missing Parent Path Population** — Comment in [storage-dbus/src/disks/volume.rs:651](../storage-dbus/src/disks/volume.rs#L651) says "TODO: Populate when building hierarchy" but is never implemented, breaking UI tree construction in some cases
...
**storage-ui** (`storage-ui/src/main.rs`)
...
**storage-ui client modules** (`client/{disks,partitions,filesystems,luks,lvm,btrfs,image}.rs`):
...
2. **storage-dbus** (library): All UDisks2 D-Bus operations
...
**Phase 1: Move UDisks2 operations to storage-dbus**

Add operation functions to storage-dbus public API:
```rust
// In storage-dbus/src/lib.rs or operations.rs
...
async fn create_partition(
    ...
    // 2. Call storage-dbus (NO D-Bus operations here)
...
- [ ] All UDisks2 D-Bus operations in storage-dbus
- [ ] All file/process operations in storage-sys (new crate)
- [ ] `cargo check` passes
- [ ] Tests can mock storage-dbus without UDisks2 daemon
...
[storage-dbus/src/disks/volume.rs:44](../storage-dbus/src/disks/volume.rs#L44):
```rust
...
[storage-dbus/src/disks/volume.rs:296](../storage-dbus/src/disks/volume.rs#L296):
```rust
...
- [volume.rs](../storage-dbus/src/disks/volume.rs): lines 296, 305, 483, 491, 500, 515, 536, 557
- [volume_model/partition.rs](../storage-dbus/src/disks/volume_model/partition.rs): lines 17, 69, 90
- [volume_model/filesystem.rs](../storage-dbus/src/disks/volume_model/filesystem.rs): lines 12, 30, 46, 68, 90
...
[storage-ui/src/models/ui_drive.rs:206-208](../storage-ui/src/models/ui_drive.rs#L206-L208):
```rust
...
Similar pattern in [storage-ui/src/models/helpers.rs:93-94](../storage-ui/src/models/helpers.rs#L93-L94):
```rust
...
**Acceptance Criteria:**
- [ ] No `Runtime::new()` calls in storage-ui
...
[storage-dbus/src/disks/volume.rs:651](../storage-dbus/src/disks/volume.rs#L651):
```rust
...
The UI relies on this in [storage-ui/src/models/helpers.rs](../storage-ui/src/models/helpers.rs) to call `build_volume_tree()`. Without correct parent_path:
...
- [storage-btrfs/src/subvolume.rs:68](../storage-btrfs/src/subvolume.rs#L68): `path.strip_prefix("<FS_TREE>/").unwrap()`
- [storage-dbus/src/disks/ops.rs](../storage-dbus/src/disks/ops.rs): 30+ `lock().unwrap()` on Mutex
- [storage-ui/src/logging.rs](../storage-ui/src/logging.rs): 8x `expect("Invalid log directive: ...")`

**Categories:**

1. **Test-only unwraps** (acceptable): storage-models tests, storage-dbus tests
...
[storage-ui/src/models/ui_drive.rs:47-50](../storage-ui/src/models/ui_drive.rs#L47-L50):
```rust
...
[storage-ui/src/models/helpers.rs:93](../storage-ui/src/models/helpers.rs#L93):
```rust
let rt = tokio::runtime::Runtime::new().ok()?;
let client = rt.block_on(FilesystemsClient::new()).ok()?;
...
- [ ] Consistent pattern used throughout storage-ui
...
[storage-ui/src/client/disks.rs:89-91](../storage-ui/src/client/disks.rs#L89-L91):
```rust
...
//! Once storage-dbus returns storage-models types directly, no conversion will be needed.
...
During refactor, `storage-models` crate was created, but `storage-dbus` still returns old `DriveModel` types. The plan was to update `storage-dbus` in "Phase 3A" to return storage-models types directly.

**Current Status:**

Phase 3A **was completed** according to implementation logs, but this conversion module still exists. Likely:
1. Some storage-dbus paths return storage-models types (new API)
...
1. Update all `storage-dbus` public APIs to return storage-models types
2. Remove `pub struct DriveModel` from storage-dbus (make it internal-only)
...
- [ ] All storage-dbus public functions return storage-models types
- [ ] Tests pass
...
1. [storage-dbus/src/disks/volume.rs:651](../storage-dbus/src/disks/volume.rs#L651):
   ```rust
   parent_path: None, // TODO: Populate when building hierarchy
   ```
   **Status:** Critical bug (covered in GAP-004)

2. [storage-ui/src/ui/volumes/update/partition.rs:195](../storage-ui/src/ui/volumes/update/partition.rs#L195):
   ```rust
   // TODO: Implement flag checking methods on PartitionInfo
   ```
   **Context:** UI needs to check if partition is bootable/system/hidden

3. [storage-ui/src/ui/volumes/update/encryption.rs:126](../storage-ui/src/ui/volumes/update/encryption.rs#L126):
   ```rust
   // TODO: Implement get_encryption_options_settings via client
   ```
   **Context:** Edit encryption options UI not wired up

4. [storage-ui/src/ui/volumes/update/encryption.rs:301](../storage-ui/src/ui/volumes/update/encryption.rs#L301):
   ```rust
...
[storage-dbus/src/disks/ops.rs](../storage-dbus/src/disks/ops.rs) has 30+ instances:
...
[storage-ui/src/client/error.rs:26-55](../storage-ui/src/client/error.rs#L26-L55):
```rust
...
Example: [storage-ui/src/client/filesystems.rs:141-150](../storage-ui/src/client/filesystems.rs#L141-L150):
```rust
pub async fn format(
    &self,
    device: &str,
...
[storage-ui/src/models/ui_drive.rs:97-125](../storage-ui/src/models/ui_drive.rs#L97-L125):
```rust
...
1. **What is the long-term plan for storage-dbus?**
