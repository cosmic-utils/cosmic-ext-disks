...
### GAP-004 — Partition segmentation uses hacks (offset and trailing bytes)
- **Type:** Correctness
- **Severity:** High
- **Impact:** Partition bar/segment display can misrepresent actual disk layout; users may make destructive choices.
- **Evidence (facts):**
  - Offset heuristic: [disks-ui/src/views/volumes.rs#L168-L176](../../disks-ui/src/views/volumes.rs#L168-L176)
  - “Hide weird end portion” hack: [disks-ui/src/views/volumes.rs#L189-L190](../../disks-ui/src/views/volumes.rs#L189-L190)
- **Repro/Trace:** Any drive where partitions don’t align with assumptions can show “missing”/hidden space.
- **Suggested Fix:** Base segmentation on authoritative partition table metadata and represent all space; remove magic constants, add debug traces for anomalies.
- **Acceptance Criteria:** Segment view reflects full disk size and partition offsets/sizes without hard-coded hiding.
