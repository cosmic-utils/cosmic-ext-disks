...
- **Severity:** High
- **Impact:** On msdos/MBR disks, create partition likely fails with “Unsupported partition table type” or “not compatible” even if DOS types are selected.
- **Evidence (facts):**
  - `create_partition` matches only `"msdos"` (not `"dos"`) and rejects other types: [disks-dbus/src/disks/drive.rs#L309-L346](../../disks-dbus/src/disks/drive.rs#L309-L346)
  - DOS partition type catalog uses `"dos"` table type: [disks-dbus/src/partition_type.rs#L150-L190](../../disks-dbus/src/partition_type.rs#L150-L190)
  - Valid names helper matches `"dos"` (not `"msdos"`): [disks-dbus/src/partition_type.rs#L68-L84](../../disks-dbus/src/partition_type.rs#L68-L84)
  - UI segment construction uses `drive.partition_table_type`: [disks-ui/src/views/volumes.rs#L172-L181](../../disks-ui/src/views/volumes.rs#L172-L181)
- **Inference:** Even if UDisks2 reports `"dos"`, this code path won’t recognize it; and the compatibility check will also fail when comparing `"dos"` vs `"msdos"`.
- **Suggested Fix:** Normalize table type strings (treat `dos` and `msdos` equivalently) and ensure the `partition_info.table_type` comparison uses the normalized value.
- **Acceptance Criteria:**
  - On DOS/MBR disks, either partition creation is disabled with a clear explanation, or it succeeds end-to-end.
  - No mismatched `dos/msdos` string comparisons remain on the critical path.
