...
### disks-ui (UI Application)
**Entrypoints:**
- [disks-ui/src/main.rs](disks-ui/src/main.rs) — Application bootstrap
- [disks-ui/src/app.rs](disks-ui/src/app.rs) — Legacy entry (deprecated by ui-refactor)

**Core Structure (post-refactor):**
- [disks-ui/src/ui/app/](disks-ui/src/ui/app/) — Application model (state, message, update, view, subscriptions)
- [disks-ui/src/ui/sidebar/](disks-ui/src/ui/sidebar/) — Navigation sidebar component
- [disks-ui/src/ui/volumes/](disks-ui/src/ui/volumes/) — Volume management UI (disk header, segments, actions)
- [disks-ui/src/ui/dialogs/](disks-ui/src/ui/dialogs/) — Dialog state and views
- [disks-ui/src/utils/](disks-ui/src/utils/) — Utility functions (segments computation, UI helpers)

**Critical Files:**
- [disks-ui/src/ui/app/view.rs](disks-ui/src/ui/app/view.rs) — 806 lines (largest)
- [disks-ui/src/ui/volumes/update/encryption.rs](disks-ui/src/ui/volumes/update/encryption.rs) — 498 lines
- [disks-ui/src/utils/segments.rs](disks-ui/src/utils/segments.rs) — 453 lines

### disks-dbus (DBus Abstraction)
**Public API:**
- [disks-dbus/src/lib.rs](disks-dbus/src/lib.rs) — Glob re-exports (anti-pattern)
- [disks-dbus/src/disks/mod.rs](disks-dbus/src/disks/mod.rs) — Glob re-exports (anti-pattern)

**Core Models:**
- [disks-dbus/src/disks/drive/](disks-dbus/src/disks/drive/) — Drive discovery, actions, SMART
- [disks-dbus/src/disks/volume_model.rs](disks-dbus/src/disks/volume_model.rs) — 944 lines (needs split)
- [disks-dbus/src/disks/volume.rs](disks-dbus/src/disks/volume.rs) — 629 lines (volume tree structures)

**Partition Types:**
- [disks-dbus/src/partition_types/gpt.rs](disks-dbus/src/partition_types/gpt.rs) — **1503 lines** (massive data catalog)
- [disks-dbus/src/partition_types/dos.rs](disks-dbus/src/partition_types/dos.rs) — 357 lines
...
**Location:**
- [disks-ui/i18n/en/cosmic_ext_disks.ftl](disks-ui/i18n/en/cosmic_ext_disks.ftl) (lines vary)
- [disks-ui/i18n/sv/cosmic_ext_disks.ftl](disks-ui/i18n/sv/cosmic_ext_disks.ftl) (lines vary)
...
**File:** [disks-ui/src/utils/mod.rs](disks-ui/src/utils/mod.rs)
```rust
mod segments;
mod ui;
...
**File:** [disks-dbus/src/lib.rs](disks-dbus/src/lib.rs)
```rust
pub use dbus::*;
pub use disks::*;
pub use format::*;
...
**File:** [disks-dbus/src/disks/mod.rs](disks-dbus/src/disks/mod.rs) (lines 12-21)
```rust
...
**Suggested Fix:**
1. **For `disks-ui/src/utils/mod.rs`:**
...
3. **For `disks-dbus/src/disks/mod.rs`:**
...
**Evidence:**
- [disks-dbus/src/disks/volume_model.rs](disks-dbus/src/disks/volume_model.rs) — 944 lines
- Contains:
...
**Evidence:**
- [disks-dbus/src/partition_types/gpt.rs](disks-dbus/src/partition_types/gpt.rs) — **1503 lines**
- Contains ~150+ hardcoded `PartitionTypeInfo` structs
- Similar issue in [disks-dbus/src/partition_types/dos.rs](disks-dbus/src/partition_types/dos.rs) — 357 lines
...
**File:** [disks-ui/src/utils/ui.rs](disks-ui/src/utils/ui.rs) (line 1)
```rust
#![allow(dead_code)]
```
...
**File:** [disks-dbus/src/disks/volume_model.rs](disks-dbs/src/disks/volume_model.rs) (lines 866-872)
```rust
...
**File:** [disks-dbus/src/partition_types/mod.rs](disks-dbus/src/partition_types/mod.rs) (line 55)
```rust
//TODO: wrap name with gettext call
```
No issue tracker reference, no explanation of why not done yet.

**File:** [disks-dbus/src/format.rs](disks-dbus/src/format.rs) (line 28)
```rust
let bytes_str = bytes.to_formatted_string(&Locale::en); //TODO: Accept locale as a parameter.
```

**File:** [disks-dbus/src/disks/volume_model.rs](disks-dbus/src/disks/volume_model.rs) (lines 646, 667, 875)
```rust
...
**File:** [disks-ui/src/ui/dialogs/view/image.rs](disks-ui/src/ui/dialogs/view/image.rs) (line 151)
```rust
...
**File:** [disks-dbus/src/disks/volume_model.rs](disks-dbus/src/disks/volume_model.rs) (line 289)
```rust
println!("{type_str}");
```
...
- [disks-dbus/src/disks/drive/mod.rs](disks-dbus/src/disks/drive/mod.rs)
...
- `disks-ui/src/ui/volumes/` has no `mod.rs`, just sibling files
...
3. **What is the long-term plan for `disks-ui/src/app.rs`?** — File still exists but likely deprecated by ui-refactor. Should it be deleted?
...
**Full read:**
- [disks-ui/src/utils/mod.rs](disks-ui/src/utils/mod.rs)
- [disks-ui/src/utils/segments.rs](disks-ui/src/utils/segments.rs)
- [disks-ui/src/utils/ui.rs](disks-ui/src/utils/ui.rs)
- [disks-ui/src/logging.rs](disks-ui/src/logging.rs)
- [disks-dbus/src/lib.rs](disks-dbus/src/lib.rs)
- [disks-dbus/src/disks/mod.rs](disks-dbus/src/disks/mod.rs)
- [disks-dbus/src/disks/volume_model.rs](disks-dbus/src/disks/volume_model.rs) (partial)

**Scanned:**
- [disks-ui/src/ui/volumes/view.rs](disks-ui/src/ui/volumes/view.rs)
- [disks-ui/src/ui/app/view.rs](disks-ui/src/ui/app/view.rs)
- [disks-dbus/src/partition_types/gpt.rs](disks-dbus/src/partition_types/gpt.rs)
- [disks-dbus/src/partition_types/dos.rs](disks-dbus/src/partition_types/dos.rs)
...
1. **"Not mounted" (3 occurrences)**
   - [disks-ui/src/ui/app/view.rs](disks-ui/src/ui/app/view.rs) lines 371, 497, 515
   ```rust
   widget::text::caption("Not mounted").into()
   ```

2. **"Can create partition"**
   - [disks-ui/src/ui/app/view.rs](disks-ui/src/ui/app/view.rs) line 787
   ```rust
   let available_text = widget::text::caption("Can create partition");
   ```

3. **"Offset: {}" format string**
   - [disks-ui/src/ui/app/view.rs](disks-ui/src/ui/app/view.rs) line 783
   ```rust
...
4. **"UUID: {}" format string**
   - [disks-ui/src/ui/app/view.rs](disks-ui/src/ui/app/view.rs) line 478
   ```rust
...
5. **Volume type labels (4 strings)**
   - [disks-ui/src/ui/app/view.rs](disks-ui/src/ui/app/view.rs) lines 337-340
   ```rust
...
1. Add missing keys to `disks-ui/i18n/en/cosmic_ext_disks.ftl`:
   ```fluent
...
3. Add Swedish translations to `disks-ui/i18n/sv/cosmic_ext_disks.ftl`
...
**Scanned:**
- [disks-ui/src/ui/volumes/view.rs](disks-ui/src/ui/volumes/view.rs)
- [disks-ui/src/ui/app/view.rs](disks-ui/src/ui/app/view.rs) — **untranslated strings found**
- [disks-dbus/src/partition_types/gpt.rs](disks-dbus/src/partition_types/gpt.rs)
- [disks-dbus/src/partition_types/dos.rs](disks-dbus/src/partition_types/dos.rs)
